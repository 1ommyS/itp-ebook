...
* **Плюсы:**
    ...
    * Компактная специализация для сравнений на равенство.
* **Минусы:**
...
* **Пример:**
  ...
  ```sql
  -- Индекс для адресов и сетевых диапазонов
  CREATE INDEX idx_network_address
    ON network USING spgist (address inet_ops);
  ```
* **Плюсы:**

    * Эффективен для некоторых нетривиальных задач: IP-сетей, геометрии и разбиения пространства.
...
* **Структура:** хранит компактную сводку для настраиваемого диапазона страниц таблицы; по умолчанию один диапазон охватывает много соседних страниц.
* **Поддерживает:** подходящие operator classes, но эффективен прежде всего для физически коррелированных данных — например, времени последовательной загрузки.
* **Синтаксис:**
